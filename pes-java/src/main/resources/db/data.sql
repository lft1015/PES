...
-- ================================================
-- 菜单与按钮权限数据（三层树结构）
-- type: 0=目录(分组，不可点击)  1=菜单(可访问页面)  2=按钮(操作权限)
-- ================================================
...
-- ==================== 系统管理（目录） ====================
(1, 0, '系统管理', '', '', 'Setting', 'system:manage', 2, 0, 1),
-- 用户管理
...
-- ==================== 日志管理（目录） ====================
(18, 0, '日志管理', '', '', 'Document', 'log:manage', 3, 0, 1),
-- 操作日志（operlog:* 独立标识，与登录日志按钮权限解耦）
(19, 18, '操作日志', '/log/operation', 'log/OperationLog', 'Tickets', 'operlog:list', 1, 1, 1),
(23, 19, '日志查询', '', '', '', 'operlog:view', 1, 2, 1),
(24, 19, '日志删除', '', '', '', 'operlog:delete', 2, 2, 1),
(25, 19, '日志清空', '', '', '', 'operlog:clear', 3, 2, 1),
(28, 19, '日志批量删除', '', '', '', 'operlog:batch', 4, 2, 1),
-- 登录日志（loginlog:* 独立标识）
(20, 18, '登录日志', '/log/login', 'log/LoginLog', 'Stamp', 'loginlog:list', 2, 1, 1),
(26, 20, '日志查询', '', '', '', 'loginlog:view', 1, 2, 1),
(21, 20, '日志删除', '', '', '', 'loginlog:delete', 2, 2, 1),
(27, 20, '日志清空', '', '', '', 'loginlog:clear', 3, 2, 1),
(29, 20, '日志批量删除', '', '', '', 'loginlog:batch', 4, 2, 1);
...
-- ================================================
-- 角色-菜单关联
-- 注意：超级管理员拥有所有菜单（含目录、菜单、按钮），其他角色按需分配
-- 目录（type=0）也必须关联，否则左侧菜单不会显示该目录分组
-- ================================================
...
-- 审计员(5)：仪表盘 + 日志管理目录 + 全部日志
(60, 5, 18), (61, 5, 19), (62, 5, 20),
(63, 5, 23), (64, 5, 24), (65, 5, 25), (66, 5, 26), (67, 5, 21), (68, 5, 27),
(71, 5, 22),

-- 批量删除按钮（操作日志/登录日志）分配给所有拥有日志删除能力的角色
(72, 1, 28), (73, 3, 28), (74, 4, 28), (75, 5, 28),
(76, 1, 29), (77, 3, 29), (78, 4, 29), (79, 5, 29);
